...
echo "[6/5] Перезапуск сервера ..."
"$APP_DIR/scripts/stop_gps_logger.sh" || true
"$APP_DIR/scripts/start_gps_logger.sh"

echo "[7/5] Smoke-check /api/point-comment ..."
...
if [ "$POINT_COMMENT_STATUS" = "404" ]; then
  echo "[ERR] Route /api/point-comment not found after restart"
  cat /tmp/gps_logger_point_comment_smoke.json || true
  exit 1
fi
if [ "$POINT_COMMENT_STATUS" = "000" ]; then
  echo "[ERR] Smoke-check request failed"
  exit 1
fi
echo "[OK] /api/point-comment доступен, status=$POINT_COMMENT_STATUS"

echo
echo "[DONE] Патч установлен и проверен."
echo "Открой:"
